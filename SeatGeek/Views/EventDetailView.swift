            ...
            HStack {
              Button("Add to My Events") {
                //TODO: Add to Event List
                  ...
                  self.dbHelper.checkExistingEventinFavorites(currentuser: self.loggedUser, event: selectedEvent!){exists, error in
                      if let error = error {
                        // Handle error
                        print("Error checking existing event in favorites: \(error.localizedDescription)")
                      }else if exists {
