  ...
  var body: some View {
    ...
    ZStack {
      ...
      VStack {
        HStack {
          ...
          VStack {
            ...
            Button(action: {
              ...
              self.dbHelper.checkExistingFriend(loggedUser: self.loggedUser, friend: newFriend) {
                exists, error in
                if let error = error {
                  // Handle error
                ...
                if exists {
                  ...
                  do {
                    try self.dbHelper.addFriend(loggedUser: self.loggedUser, friend: newFriend) {
                      success, error in
                      if success {
        ...
        if self.closestEvent != "User does not have events soon" {
          List {
            if self.dbHelper.friendsAttendingSameEvent.isEmpty {
              ...
              if self.dbHelper.friendsAttendingSameEvent.count == 1
                && self.dbHelper.friendsAttendingSameEvent[0].email == selectedUser.email
              {
    ...
    .onAppear {
      ...
      self.dbHelper.checkExistingFriend(
