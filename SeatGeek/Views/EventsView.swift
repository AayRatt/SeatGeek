...
struct EventsView: View {
  ...
  func loadDataFromAPI() {
    ...
    let websiteAddress =
      "https://api.seatgeek.com/2/events?lat=\(centerCoordinate.latitude)&lon=\(centerCoordinate.longitude)&client_id=MzQ3MjE5NjB8MTY4ODU3NjI3NC43NzAzNTYy"
  ...
  var body: some View {
    NavigationStack {
      ZStack {
        ...
        VStack {
          MapView(
            centerCoordinate: $centerCoordinate, annotations: annotations,
...
struct MapView: UIViewRepresentable {
  ...
  func updateUIView(_ view: MKMapView, context: Context) {
    ...
    if let userLocation = view.userLocation.location?.coordinate {
      let region = MKCoordinateRegion(
        center: userLocation, latitudinalMeters: 35000, longitudinalMeters: 35000)
  ...
  func updateMapRegion(view: MKMapView) {
    let region = MKCoordinateRegion(
      center: centerCoordinate, latitudinalMeters: 35000, longitudinalMeters: 35000)
    view.setRegion(region, animated: true)
  }
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
      ...
      if let dequeuedView = mapView.dequeueReusableAnnotationView(withIdentifier: identifier) {
        ...
        annotationView = MKPinAnnotationView(
          annotation: eventAnnotation, reuseIdentifier: identifier)
...
class EventAnnotation: MKPointAnnotation {
  ...
  init(event: Event, onTap: ((Event) -> Void)?) {
    ...
    self.coordinate = CLLocationCoordinate2D(
      latitude: event.venue.location.lat ?? 0, longitude: event.venue.location.lon ?? 0)
